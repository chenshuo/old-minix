 ...
 */
  ...
  /* Skip over leading blanks if in fill mode. we indent later. since
   * leadbl does a robrk, do it if in .nf mode */
  if (dc.fill == YES) {
	if (*p == ' ' || *p == '\n' || *p == '\r') leadbl(p);
  } else
	robrk();
  ...
  if (dc.ulval > 0) {
	/* Underline (.ul) 
	 * Because of the way underlining is handled, MAXLINE should be
	 * declared to be three times larger than the longest
	 * expected input line for underlining.  Since many of the
	 * character buffers use this parameter, a lot of memory can
	 * be allocated when it may not really be needed.  A MAXLINE
	 * of 180 would allow about 60 characters in the output line
	 * to be underlined (remember that only alphanumerics get
	 * underlined - no spaces or punctuation). */
  ...
  if (dc.ceval > 0) {
	...
	/* Get a word and put it out. increment ptr to the next word. */
 ...
 */
  ...
  h[++i] = EOS;
  if (strlen(h) > 2) {
	for (i = 0; i < pg.offset; ++i) prchar(' ', out_stream);
  }
  putlin(h, out_stream);
}
 ...
 */
  ...
  if (((co.outp > 0) && ((co.outw + w - co.outesc) > llval))
	...
	if (dc.juval == YES) {
		...
		/* Do not take in the escape char of the word that
		 * didn't fit on this line anymore */
		co.outesc -= countesc(wrdbuf);

		/* Check whether last word was end of sentence and
	...
	/* Break line, output it, and reset all co members. reset esc
	 * count. */
	robrk();
  ...
  /* Copy the current word to the out buffer which may have been reset */
  p0 = wrdbuf;
...
/*	spread			*/
/*------------------------------*/
void spread(p, outp, nextra, outwds, escapes)
register char *p;
int outp;
int nextra;
int outwds;
 ...
 */
  ...
  /* Quick sanity check... */
  if ((nextra + escapes < 1) || (outwds < 2)) return;
