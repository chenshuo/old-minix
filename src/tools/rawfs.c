...
/* rawfs is based on readfs by Paul Polderman */
...
/* The following code handles two file system types: 
 *    Version 1 with small inodes and 16-bit disk addresses
 *    Version 2 with big inodes and 32-bit disk addresses.
 */

/* Version 1 or 2 file system constants. */
...
off_t r_super()
{
/* Initialize variables, return size of a file system in blocks,
 * (zero on error).
 */

  readblock((off_t) SUPER_BLOCK, scratch);	  /* read superblock */
 ...
 */
  ...
  if (zone < (zone_t) nr_indirects) {	/* single indirect block */
	...
	ind_zone = super.s_magic == SUPER_V2 ?
		fsbuf(dindir).b_v2_ind[(int) (zone/(zone_t) nr_indirects)] :
		fsbuf(dindir).b_v1_ind[(int) (zone/(zone_t) nr_indirects)];
	zone %= (zone_t) nr_indirects;
  }
...
ino_t r_lookup(cwd, path)
Ino_t cwd;
char *path;
/* Translates a pathname to an inode number.  This is just a nice utility
 * function.  It needs r_stat and r_readdir. It starts from 'cwd' unless
 * the path starts with '/', in which case it starts from ROOT_INO.
 */
{
