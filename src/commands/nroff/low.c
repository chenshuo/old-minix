  ...
  if (aval > 0) {
	...
	if ((aval % 5 == 0) && (aval % 10 != 0))	/* 5 */
		...
		if (rem == 9) {	/* 9 */
			...
			for (j = 0; j < rem; j++) c[i++] = 'I';
		}
	}

	aval /= 10;
	if (aval == 0) goto done_100;
	...
	if (rem == 4) {
		...
		for (j = 0; j < rem; j++) c[i++] = 'X';
	} else {
		for (j = 0; j < rem - 5; j++) c[i++] = 'X';
		c[i++] = 'L';
	}
  }
done_100:
  ...
  if (aval > 0) {
	...
	if (rem == 5) {
		...
		for (j = 0; j < rem; j++) c[i++] = 'C';
	} else if (rem == 9) {
		c[i++] = 'M';
		c[i++] = 'C';
	} else if (rem < 9) {
		for (j = 0; j < rem - 5; j++) c[i++] = 'C';
		c[i++] = 'D';
	}
  }
  aval /= 10;
 ...
 */
  ...
  for (i = 0; i < len; i++) {
	p[i] = c[i];
	if (isalpha(p[i])) p[i] = tolower(c[i]);
  }
 ...
 */
  ...
  /* 1 based: 
   * 0	0 1	A 25	Z 26	AA 51 AZ 52 AAA ... */
  if (aval == 0)
	c[i++] = '0';
  else if (aval < 27) {
	c[i++] = aval - 1 + 'A';
