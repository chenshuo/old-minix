/* escape.c - Escape and special character input processing portion of nroff */
  ...
  while (*s != EOS) {
	if (*s != dc.escchr) {
		...
		*t = *s;
		...
		/* It returns num char to skip and sets c to the
  ...
  *s = '\0';
  switch (fnt) {
	...
	break;
      ...
      case 'I':			/* Times italic */
	if (dc.dofnt == YES) strcpy(s, s_italic);
	dc.lastfnt = dc.thisfnt;
	dc.thisfnt = 2;
	break;

      case 'B':			/* Times bold */
	if (dc.dofnt == YES) strcpy(s, s_bold);
	dc.lastfnt = dc.thisfnt;
	dc.thisfnt = 3;
	break;
 ...
 */
  ...
  for (i = 0; i < MAXREGS; i++) {
	prname = rg[i].rname;
	if (*prname == *name && *(prname + 1) == *(name + 1)) break;
  }
