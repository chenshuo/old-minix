...
_PROTOTYPE(void sigcatch, (int sig));
_PROTOTYPE(char agetchar, (int fd));
_PROTOTYPE(char *readline, (int fd));
_PROTOTYPE(void decode, (char *text, ENTRY *ep));
_PROTOTYPE(int readdefs, (char *name, char *label));
_PROTOTYPE(void select_speed, (void));
_PROTOTYPE(void showfile, (char *name));
_PROTOTYPE(void wtmp, (char *line));
_PROTOTYPE(int areadch, (void));
_PROTOTYPE(void do_getty, (char *name));
_PROTOTYPE(void do_login, (char *name));
_PROTOTYPE(int getspeed, (char *spd));
_PROTOTYPE(void do_stty, (char *parms));
_PROTOTYPE(void do_carrier, (void));
...
int fd;
{
/* Read a character, or E_O_F if zero chars read. */
...
char *line;			/* tty device name */
{
  ...
  close(fd);
  recoff = recno * (off_t) sizeof(struct utmp);
  ...
  if ((fd = open(UTMP, O_WRONLY)) > 0) {
	if (lseek(fd, recoff, SEEK_SET) != (off_t)-1) {
		write(fd, (char *) &entry, sizeof(struct utmp));
	}
	close(fd);
  }
